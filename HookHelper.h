...
#define GET_ADDRESS(addr) ((reinterpret_cast<size_t>(GetModuleHandleW(NULL))) + addr)
#define GET_DLL_ADDRESS(dll, addr) ((reinterpret_cast<size_t>(GetModuleHandleW(dll))) + addr)
...
inline LONG registerHook(PVOID* orig, PVOID hook) {
    ...
    hooks.push_back(std::make_tuple(orig, hook));
...
inline LONG unregisterHook(PVOID* orig, PVOID hook) {
    ...
    hooks.erase(std::remove(hooks.begin(), hooks.end(), std::make_tuple(orig, hook)), hooks.end());
    DetourTransactionCommit();
    return att;
}
